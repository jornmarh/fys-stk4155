...
n_hidden_neurons = 50
n_hidden_layers = 1
activations = ["Sigmoid", "RELU", "leaky-RELU"]
initializations = ["Xavier", "He", "He"]

n_epochs = 200
M = 10
etas = [1e-3, 1e-3, 1e-3]
lambdas = [1e-8, 0.1, 0.1]
...
for i in range(len(activations)):
    for train_indexes, test_indexes in kfold.split(X):
        ...
        network1 = NN(X_train, X_test, t_train, t_test, n_hidden_layers, n_hidden_neurons, activations[i], initializations[i])
        acs_epochs = network1.train(n_epochs, M, etas[i], lambdas[i])

        scores_cvd.append(acs_epochs)

    accuracy_epochs = np.mean(np.asarray(scores_cvd), axis=0)
    functions.append(accuracy_epochs)

epochs = np.arange(1, n_epochs+1)
plt.plot(epochs, functions[0], label='Sigmoid')
plt.plot(epochs, functions[1], label='RELU')
plt.plot(epochs, functions[2], label='leaky-RELU')
#plt.ylim(0.8, 1)
#plt.xlim(85,125)
plt.ylabel("Accuracy score")
plt.xlabel("Iterations(epochs)")
plt.title("Accuracy convergence of Sigmoid, RELU and leaky-RELU activation")
plt.legend()
plt.show()
