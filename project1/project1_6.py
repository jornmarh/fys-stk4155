...
if (complexity == True):
    ...
    for i in range(maxdegree):
        ...
        if(cvd == True):
            # Use the KFold split method from Scikit-learn
            ...
            for train_inds, test_inds in kfold.split(X):
                ...
                # OLS prediction
                beta_ols_cvd = np.linalg.pinv(
                    xtrain.T @ xtrain) @ xtrain.T @ ztrain
                zPredict_ols_cvd = xtest @ beta_ols_cvd

                # ridge
                _I = np.eye(xtrain.shape[1], xtrain.shape[1])
                beta_ridge_cvd = np.linalg.pinv(
                    xtrain.T @ xtrain + lmd*_I) @xtrain.T @ ztrain
        ...
        if (bootstrap == True):
            X_train, X_test, z_train, z_test = train_test_split(
                X, z, test_size=0.2)
            X_train, X_test, z_train, z_test = scale(
            ...
            for boot in range(nBootstrap):
                ...
                # OLS prediction of the same test data for every bootstrap
                zPredict_ols_bootstrap[:, boot] = X_test @ beta_ols_bootstrap

                # Ridge
                _I = np.eye(X_train.shape[1], X_train.shape[1])
                beta_ridge_bootstrap = np.linalg.pinv(
                    X_.T @ X_ + lmd*_I) @ X_.T @ z_
                zPredict_ridge_bootstrap[:,
                                         boot] = X_test @ beta_ridge_bootstrap
            ...
            # OLS
            error_ols_bootstrap[i] = np.mean(np.mean(
                (z_test.reshape(-1, 1) - zPredict_ols_bootstrap)**2, axis=1, keepdims=True))
            bias_ols_bootstrap[i] = np.mean(
                (z_test.reshape(-1, 1) - np.mean(zPredict_ols_bootstrap, axis=1, keepdims=True))**2)
            var_ols_bootstrap[i] = np.mean(
                np.var(zPredict_ols_bootstrap, axis=1, keepdims=True))
            # Ridge
            error_ridge_bootstrap[i] = np.mean(np.mean(
                (z_test.reshape(-1, 1) - zPredict_ridge_bootstrap)**2, axis=1, keepdims=True))
            bias_ridge_bootstrap[i] = np.mean(
                (z_test.reshape(-1, 1) - np.mean(zPredict_ridge_bootstrap, axis=1, keepdims=True))**2)
            var_ridge_bootstrap[i] = np.mean(
                np.var(zPredict_ridge_bootstrap, axis=1, keepdims=True))
            # Lasso
            error_lasso_bootstrap[i] = np.mean(np.mean(
                (z_test.reshape(-1, 1) - zPredict_lasso_bootstrap)**2, axis=1, keepdims=True))
            bias_lasso_bootstrap[i] = np.mean(
                (z_test.reshape(-1, 1) - np.mean(zPredict_lasso_bootstrap, axis=1, keepdims=True))**2)
            var_lasso_bootstrap[i] = np.mean(
                np.var(zPredict_lasso_bootstrap, axis=1, keepdims=True))

            print("Bootstrap OLS")
            print("degree", "       error",
                  "                Bias", "                  Var")
            print("{}   {}   {}     {}".format(
                degrees[i], error_ols_bootstrap[i], bias_ols_bootstrap[i], var_ols_bootstrap[i]))

            print("Bootstrap ridge")
            print("{}   {}   {}     {}".format(
                degrees[i], error_ridge_bootstrap[i], bias_ridge_bootstrap[i], var_ridge_bootstrap[i]))

            print("Bootstrap Lasso")
            print("{}   {}   {}     {}".format(
                degrees[i], error_lasso_bootstrap[i], bias_lasso_bootstrap[i], var_lasso_bootstrap[i]))
